...
// init variable used by automarker
if($.hasOwnProperty('_automarker') === false){
	$._automarker = {generatedIds:[]}
}

// register function to generate new object id for automarker
$._automarker.generateId = function(){
	var result = '';
	var characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
	for (var i = 0; i < 10; i++ ) {
		result += characters.charAt(Math.floor(Math.random() * characters.length));
	}
	if($._automarker.generatedIds.indexOf(result) >= 0) {
		result = $._automarker.generateId();
	}
	$._automarker.generatedIds.push(result);
	return result;
}

// replacer function to pass to ExtendJSON.stringify preventing infinite loop for $ objects
function internal_variables_replacer(key, value){if(key !== "tmp" && key !== "_automarker"){return value}}
